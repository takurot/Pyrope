...
| **P10-18** | [Core] | **IVF-PQ (Product Quantization)**<br>Implement Product Quantization for IVF index. Reduces memory by 4-8x, enabling 1M+ vector support on limited memory. | P10-8 | [x] |
| **P10-19** | [Core] | **HNSW SIMD & Memory Optimization**<br>Optimize HNSW `Dist` with SIMD (Vector256) and transition `Node` graph to unmanaged/pinned memory (off-heap) to reduce GC pressure and pointer chasing. | P10-17 | [x] |
