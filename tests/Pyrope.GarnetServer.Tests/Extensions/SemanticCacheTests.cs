...
namespace Pyrope.GarnetServer.Tests.Extensions
{
    public class SemanticCacheTests : IDisposable
    {
        ...
        public SemanticCacheTests()
        {
            ...
            _server.Register.NewCommand("VEC.SEARCH", Garnet.server.CommandType.Read, new Pyrope.GarnetServer.Extensions.VectorCommandSet(Pyrope.GarnetServer.Extensions.VectorCommandType.Search, resultCache, policy, metrics, lsh, tenantAuthenticator: _tenantAuthenticator), new Garnet.server.RespCommandsInfo { Command = (Garnet.server.RespCommand)Pyrope.GarnetServer.Extensions.VectorCommandSet.VEC_SEARCH, Name = "VEC.SEARCH" });
            _server.Register.NewCommand("VEC.STATS", Garnet.server.CommandType.Read, new Pyrope.GarnetServer.Extensions.VectorCommandSet(Pyrope.GarnetServer.Extensions.VectorCommandType.Stats, null, null, metrics, tenantAuthenticator: _tenantAuthenticator), new Garnet.server.RespCommandsInfo { Command = (Garnet.server.RespCommand)Pyrope.GarnetServer.Extensions.VectorCommandSet.VEC_STATS, Name = "VEC.STATS" });
        ...
        public void Search_SimilarVector_ShouldHitCache()
        {
            ...
            _db.Execute("VEC.ADD", tenant, index, id, "VECTOR", JsonSerializer.Serialize(vector), "API_KEY", TenantApiKey);
            ...
            _db.Execute("VEC.SEARCH", tenant, index, "TOPK", "5", "VECTOR", JsonSerializer.Serialize(q1), "API_KEY", TenantApiKey);

            // Check Stats: 1 Miss
            var stats1 = (string?)_db.Execute("VEC.STATS", tenant, "API_KEY", TenantApiKey);
            ...
            _db.Execute("VEC.SEARCH", tenant, index, "TOPK", "5", "VECTOR", JsonSerializer.Serialize(q2), "API_KEY", TenantApiKey);

            // Check Stats: 1 Hit (L1)
            var stats2 = (string?)_db.Execute("VEC.STATS", tenant, "API_KEY", TenantApiKey);
